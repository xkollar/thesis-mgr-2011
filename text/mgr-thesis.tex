...
\begin{align*}
	...
	\tau     & ::= \forall \, a^n \in \kappa^n \, . \, \tau ~|~ \sigma \\
	\kappa^n & ::= \overbrace{(\kappa,\dots,\kappa)}^n \\
	\kappa   & ::= c ~|~ \kindStar ~|~ \kappa \to \kappa ~|~ \kappa \cap \kappa \\
\end{align*}
...
\begin{itemize}
	...
	\item \(kappa^n\) represents \(n\)-parametric type class (special case for \(n = 1\) is what we had before).
\end{itemize}
...
class for containers~-- \texttt{Container}. Traditionally, it would look like this.
\begin{code}
class Container c where
    empty  :: c a
    insert :: a -> c a -> c a
\end{code}
but as we have seen sooner (when\dots removing TC infrastructure?) this forces
us to have \texttt{a}-variable quantified universally.

And \texttt{Container} as multi-parametric class.
\begin{code}
